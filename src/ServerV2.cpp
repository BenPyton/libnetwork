...
namespace net{
	...
	void ServerV2::launch(string _addr, unsigned short _port)
	{
		if (m_running)
		{
		...
		m_running = true;
		RunThreads();
		m_threadServerAccept = thread(&ServerV2::_RunAcceptClient, this);
	}

	void ServerV2::shutdown()
	{
		if (!m_running)
		{
	...
	void ServerV2::send(Socket * _client, string _msg)
	{
		if (m_running)
		{
	...
	void ServerV2::broadcast(string _msg)
	{
		send(NULL, _msg);
	}

	void ServerV2::_RunAcceptClient()
	{
		fd_set acceptfds;
		timeval timeout;
